...
class GhostFollowersViewController: UIViewController, FollowersTableKind  {
  ...
  // deinit {}
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Ghost Followers of " + self.igp.userName
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if  let abc = segue.destinationViewController as? HelpInfoViewController {
            abc.info = "Followers Info Goes Here"
        }
        else if segue.identifier == "GoBattleSegueid" {
                if let dvc = segue.destinationViewController as? BattleViewController {
                    dvc.igp = self.igp
                    dvc.followers = tableView.mutualFollowerPeople
                }
        }
    }
}// end of vc
