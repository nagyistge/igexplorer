...
class TopPostsByCommentsViewController: UITableViewController,PostsTableKind {
  ...
  // deinit {}
    ...
    override func viewDidLoad() {
        ...
        let (_freqs,commentcount) = Instagram.computeFreqCountOfCommentersForPosts(posts)
        freqs = _freqs // unpack
        self.navigationItem.title = "Top Posts by Comments by " + self.igp.userName
        self.navigationItem.promptUser("Total posts: \(posts.count) comments:\(commentcount)")
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let commentscount = (ff.comments != nil) ? ff.comments!.count : 0
        let  dateFormatter = Globals.shared.dateFormatter
        cell.textLabel!.text = ff.caption
        if let dd = Double(ff.createdTime) {
            cell.detailTextLabel!.text = IGDateSupport.dayStringFromTime(dd,dateFormatter: dateFormatter) + " " + IGDateSupport.timeStringFromUnixTime(dd,dateFormatter: dateFormatter) + " comments: \(commentscount)"
        }
        if ff.thumbPic != "" {
